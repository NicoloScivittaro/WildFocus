...
Note: the `{ timeout: 5000 }` options (and the `20000`ms test-level timeout on the last test) matter once Task 30 wires `framer-motion` into the lazy-loaded Home chunk — under full-suite load, that chunk's first import can take longer than Testing Library's default 1000ms `findBy*` timeout, which is flaky rather than a real bug. Apply this same bump if Task 30 is implemented before this file is written, or come back and add it right after.
